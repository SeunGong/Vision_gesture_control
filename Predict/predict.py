...
while True:
    ...
    #################### Predict hands ####################
    results_hands = model_hands(pose_color_image, conf=0.8, verbose=False)  
    if results_hands is not None:
        for r in results_hands:
            ...
            for number_box, box in enumerate(boxes):  
                ...
                cv2.rectangle(pose_color_image, (x1, y1), (x2, y2),(0, 0, 255), thickness=2, lineType=cv2.LINE_4)
                cv2.putText(pose_color_image, f"Depth: {depth_hand}", (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 255), 2)
                cv2.putText(pose_color_image, list_shape_hands[number_box], (box_cx, box_cy), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
