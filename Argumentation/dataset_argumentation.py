...
for filename in os.listdir(image_dir):
    if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
        ...
        # Apply the transformation
            ...
            augmented_image_path = os.path.join(image_dir, f'{filename[:-4]}_aug_{i+1}.jpg')
            ...
            if os.path.exists(label_path):
                with open(label_path, 'r') as f:
                    labels = f.read()
                augmented_label_path = os.path.join(label_dir, f'{filename[:-4]}_aug_{i+1}.txt')
                with open(augmented_label_path, 'w') as f:
                    f.write(labels)

            print(f'Processed and saved augmented image and label for {augmented_label_path}')
